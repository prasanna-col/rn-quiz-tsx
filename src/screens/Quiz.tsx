...
import { getquestiojns } from '../utils/api';
    ...
    <View style={{ flex: 1, padding: 10 }}>
      <HeaderClass title={"Quiz App"} />
      {
        !loader ? <View>
          ...
          <View style={{ marginLeft: 0 }}>
            <Text style={styles.textstyle}>Score : ({score}/{totalquestion})</Text>
          </View>
          {question.length > 0 ?
            <>
              <Questions
      ...
      <View>
        {!gameover && !loader && number != totalquestion - 1 ?
          <TouchableOpacity activeOpacity={0.8} onPress={() => nextQuestion()} >
            <Image source={Icons.NextIcon} style={{ height: 40, width: 40, resizeMode: "contain", marginLeft: "45%", margin: 20 }} />
          </TouchableOpacity>
          : (number == totalquestion - 1 ? <TouchableOpacity activeOpacity={0.8} onPress={() => startQuiz()}>
            <Image source={Icons.PlayIcon} style={{ height: 40, width: 40, resizeMode: "contain", marginLeft: "45%", margin: 20 }} />
